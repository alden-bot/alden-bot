...
import type { CommandBase, CommandContext } from './Command';
...
export class CommandManager {
	...
	public constructor(private readonly bot: AldenBot) {
		this.bot.eventManager.on(
			...
			async (event) => {
				...
				const handled = await this.dispatch(this.bot.api, message, commandName, args);
	...
	public register(command: CommandBase): boolean {
		if (!/^[a-z][a-z0-9]*$/.test(command.name)) {
			this.bot.logger.error(
				`CommandManager: Command name "${command.name}" is invalid. Use lowercase letters and numbers only.`,
			);
			return false;
		}
	...
	public async dispatch(
		...
		const ctx: CommandContext = { api, message, args, lang };
